...
try:
    ...
    # best-effort import; notebooks may run without the helper present in some contexts
    memory_utils = None
...
def extract_patches(
    ...
    save_format: str = 'png',
    # optional memory logging: when set, log memory every N patches to memory_log_path
    log_memory_every: Optional[int] = None,
    ...
    for y in range(0, height - size + 1, stride):
        for x in range(0, width - size + 1, stride):
            ...
            if patch_count % 100 == 0:
                gc.collect()
            # Optional memory logging per N patches
            try:
                if log_memory_every and memory_log_path and (patch_count % log_memory_every == 0):
                    mem = None
                    try:
                        if memory_utils:
                            mem = memory_utils.get_memory_info()
                        else:
                            mem = {'timestamp': time.time(), 'rss': None, 'vms': None, 'percent': None, 'ru_maxrss': None}
                    except Exception:
                        mem = {'timestamp': time.time(), 'rss': None, 'vms': None, 'percent': None, 'ru_maxrss': None}
                    ...
                    row = {
                        ...
                        'slide_id': slide_id if slide_id else (save_prefix if save_prefix else wsi_basename),
                        'patch_idx': patch_count,
                        'rss': mem.get('rss'),
                        'vms': mem.get('vms'),
                        'percent': mem.get('percent'),
                        'ru_maxrss': mem.get('ru_maxrss')
                    }
                    try:
                        if memory_utils:
                            memory_utils.append_memory_csv(memory_log_path, row)
                        else:
                            # fallback: append minimally
                            import csv
                            os.makedirs(os.path.dirname(memory_log_path), exist_ok=True)
                            write_header = not os.path.exists(memory_log_path)
                            keys = list(row.keys())
                            with open(memory_log_path, 'a' if not write_header else 'w', newline='') as f:
                                writer = csv.DictWriter(f, fieldnames=keys)
                                if write_header:
                                    writer.writeheader()
                                writer.writerow(row)
                    except Exception:
                        # don't let logging break extraction
                        pass
            except Exception:
                pass
